...
export default function TimelineSlot({ year, events, image }) {
  ...
  return (
    <>
      <div
        className="relative md:border-t border-t-[#C5C6CA]"
        // onMouseMove={handleMouseMove}
        // onMouseEnter={() => setIsHovered(true)}
        // onMouseLeave={() => setIsHovered(false)}
      >
        <div className="md:px-[40px] lg:px-20 hidden md:grid grid-cols-[1fr_3fr] items-center group custom-container">
        ...
        <div className="block md:hidden px-4 md:px-10 lg:px-20">
          ...
          {events.map((event, idx) => (
            <div key={idx} className="py-[16px] gap-[8px]">
              <h3 className="font-semibold text-left font-plus-jakarta text-[14px] leading-[20px] tracking-[0.8px] text-[#666666]">
                {event}
              </h3>
            </div>
          ))}
