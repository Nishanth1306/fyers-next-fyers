...
const ColumnFeaturesSection = ({ feature, isHeading = true }) => {
  ...
  return (
    <div
      className={`bg-white flex flex-col gap-[24px] md:gap-[32px] ${
      ...
      <section className="relative">
        ...
        <div className="flex flex-col px-[20px] md:hidden">
          {feature.map(({ iconSrc, iconAlt, title, description }, index) => {
            return (
              <div className="p-[16px] border-x border-b flex flex-row items-center gap-4 w-full border-[#C8C8CC]" key={index}>
                <Image
                  src={iconSrc}
                  alt={iconAlt}
                  height={32}
            ...
            {feature.map(({ iconSrc, iconAlt, title, description }, index) => {
              ...
              return (
                <div
                  key={title}
                  className={`${
                    spanFull ? "col-span-2 border-r border-[#C5C6CA]" : ""
                  } flex h-full`}
                >
                  <FeatureLayout
                    iconSrc={iconSrc}
                    ...
                    isLastInRow={
                      (index + 1) % 2 === 0 || index === feature.length - 1
                    }
                    isMobile={false}
                    index={index}
                  />
